...
def main():
    parser = argparse.ArgumentParser(description="Advanced Hello World for CI/CD Testing")
    parser.add_argument('--test', action='store_true', help='Run all functions with test values')
    parser.add_argument('--add', nargs=2, type=float, metavar=('A', 'B'), help='Add two numbers')
    parser.add_argument('--fact', type=int, metavar='N', help='Calculate factorial of N')
    parser.add_argument('--palindrome', type=str, metavar='WORD', help='Check if word is palindrome')
    ...
    if args.add:
        print(f"{args.add[0]} + {args.add[1]} = {add_numbers(args.add[0], args.add[1])}")
